...
impl BinarySearch for Vec<f64> {
    ...
    /// Если элемент в векторе отсутствует, возвращает индексы соседних элементов между которыми лежит
    /// искомый элемент ```(Some(index), Some(index))```.
    /// Если значение не найдено, возвращает ```(None, None)```.
    ///
    ///
    /// # Examples
    /// ```
    ...
    fn custom_binary_search(&self, value: f64) -> (Option<usize>, Option<usize>) {
        ...
        while left_point != right_point - 1 {
            ...
            let searched_value = *self.get(middle).unwrap();
