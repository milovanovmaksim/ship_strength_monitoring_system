...
mod tests {
    ...
    fn test_number_spatiums() {
        call_once();
        let ship_dimensions = ShipDimensions::new(125.0,20, 0.8);
        let lightweight = Lightweight::new(1750.0, ship_dimensions.clone());
        let output = lightweight.lightweight_intensity();
        assert_eq!(output.len(), ship_dimensions.number_spatiums() as usize);
    }
    ...
    fn test_lightweight_intensity() {
        call_once();
        let test_weight = 13575.0;
        let ship_dimensions = ShipDimensions::new(235.0,20, 0.65);
