...
mod tests {
    ...
    fn area_by_draft_err_test() {
        ...
        let abscissa =  -25.0;
        let frame = Frame::new(id, drafts, areas, volumes, masses, abscissa);
        assert_eq!(Err("Осадка меньше чем минимально известная осадка для данного шпангоута. Минимальная осадка: 1 метра.".to_string()), frame.area_by_draft(0.0));
        assert_eq!(Err("Осадка превысила максимально допустимое значение для данного судна. Максимальная осадка: 13.3 метра.".to_string()), frame.area_by_draft(15.0));
