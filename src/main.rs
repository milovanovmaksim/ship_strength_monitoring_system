...
fn main() {
    ...
    let ship_dimensions = ShipDimensions::from_json_file("./input_data/input_data.json".to_string()).unwrap();
    let lightweight_intensity = LightweightIntensity::new(1750.56, ship_dimensions, None);
    let lightweight_intensity_spatium_functions = lightweight_intensity.intensity();
